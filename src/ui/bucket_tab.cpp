...
QWidget* BucketTab::get_run_box() {
  ...
  boxLayout->addWidget(astar_button);
  fringe_button = new QPushButton{"Run Fringe"};
  boxLayout->addWidget(fringe_button);
  box->setLayout(boxLayout);
  return box;
}


void BucketTab::update_table_scenarios(int index) {
  if (index==-1) {
    return;
  }
  result_table->clear();
...
void BucketTab::load_retvals_to_result_table(std::vector<RetVal> retvals, int start_column) {
