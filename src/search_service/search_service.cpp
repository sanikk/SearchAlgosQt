...
#include "fringe_with_linked_pair_map.h"
#include <iostream>

SearchService::SearchService(ScenarioService& i_scenario_service) : scenarioService(i_scenario_service) {};
...
void printRoute(const std::vector<int>& route, const int map_size) { printRoute(pair_route(route, map_size)); }
...
void SearchService::runAstar(int bucket, int index) { SearchService::runAstar(10 * bucket + index); }
...
void SearchService::runFringe(int bucket, int index) { SearchService::runFringe(bucket * 10 + index); }
...
std::vector<RetVal> SearchService::runTestVersionForBucket(const int bucket) {
    ...
    for (Scenario& scenario : scenario_list) {
        auto start = std::chrono::high_resolution_clock::now();
        RetVal ret = fringe_with_linked_pair_map(scenario.start_x, scenario.start_y, scenario.goal_x, scenario.goal_y, scenarioService.get_map());
        auto end = std::chrono::high_resolution_clock::now();
        ret.timing = end - start;
        retvals.push_back(ret);
    }
